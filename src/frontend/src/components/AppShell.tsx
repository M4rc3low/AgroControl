      ...
      <aside className={`sidebar ${sidebarOpen ? 'sidebar--open' : ''}`}>
        ...
        <nav className="nav" aria-label="Navegação principal">
          ...
          {navigation.map(item => {
            ...
            return (
              <NavLink key={item.to} to={item.to} end={item.to === '/'} onClick={closeSidebar} className={({ isActive }) => `nav__link ${isActive ? 'nav__link--active' : ''} ${allowed ? '' : 'nav__link--locked'}`.trim()}>
                <Icon name={item.icon} size={19} /><span>{item.label}</span>{!allowed && <Icon name="lock" size={14} className="nav__lock" />}
              </NavLink>
            );
          })}
        </nav>
