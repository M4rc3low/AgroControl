...
app.MapGet("/", () => Results.Ok(new { service = "AgroControl.Api", status = "running", version = "0.4.0" }));
...
app.MapPost("/api/v1/auth/register", async (RegisterCommand request, AuthService authService, CancellationToken cancellationToken) =>
{
...
app.MapPost("/api/v1/auth/login", async (LoginCommand request, AuthService authService, CancellationToken cancellationToken) =>
{
    var result = await authService.LoginAsync(request, cancellationToken);
    return result.Succeeded ? Results.Ok(result.Value) : Results.Problem(statusCode: StatusCodes.Status401Unauthorized, title: "Invalid credentials", detail: result.Error);
});

var authorized = app.MapGroup("/api/v1").RequireAuthorization();
authorized.MapGet("/me", (ClaimsPrincipal principal) => Results.Ok(new { userId = principal.FindFirstValue(ClaimTypes.NameIdentifier) ?? principal.FindFirstValue("sub"), email = principal.FindFirstValue(ClaimTypes.Email) ?? principal.FindFirstValue("email"), organizationId = principal.FindFirstValue("org_id"), role = principal.FindFirstValue(ClaimTypes.Role) }));
authorized.MapGet("/organizations/current", async (ClaimsPrincipal principal, IIdentityRepository identityRepository, CancellationToken cancellationToken) =>
{
...
authorized.MapGet("/platform/entitlements", async (ClaimsPrincipal principal, ModuleAccessService moduleAccessService, CancellationToken cancellationToken) =>
{
    if (!TryGetOrganizationId(principal, out var organizationId)) return Results.Unauthorized();
...
authorized.MapGet("/platform/modules/{moduleKey}/access", async (string moduleKey, ClaimsPrincipal principal, ModuleAccessService moduleAccessService, CancellationToken cancellationToken) =>
{
    ...
    return allowed ? Results.Ok(new { module = module.ToString(), allowed = true }) : Results.Problem(statusCode: StatusCodes.Status403Forbidden, title: "Module not enabled", detail: $"The {module} module is not enabled for this organization.");
});
