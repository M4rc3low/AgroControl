...
public sealed class ExportRepository(AgroControlDbContext dbContext) : IExportRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(search))
        {
            ...
            query = query.Where(x => EF.Functions.ILike(x.OrderNumber, pattern) || EF.Functions.ILike(x.BuyerName, pattern) || EF.Functions.ILike(x.ProductDescription, pattern));
        }
    ...
    public Task<IReadOnlyList<ExportDocument>> ListDocumentsAsync(Guid organizationId, Guid orderId, CancellationToken cancellationToken = default) =>
        dbContext.ExportDocuments.AsNoTracking().Where(x => x.OrganizationId == organizationId && x.OrderId == orderId)
            .OrderBy(x => x.Type).ThenBy(x => x.CreatedAtUtc).ToListAsync(cancellationToken)
            .ContinueWith(task => (IReadOnlyList<ExportDocument>)task.Result, cancellationToken);
    ...
    public Task<IReadOnlyList<ExportCost>> ListCostsAsync(Guid organizationId, Guid orderId, CancellationToken cancellationToken = default) =>
        dbContext.ExportCosts.AsNoTracking().Where(x => x.OrganizationId == organizationId && x.OrderId == orderId)
            .OrderByDescending(x => x.IncurredOn).ThenByDescending(x => x.CreatedAtUtc).ToListAsync(cancellationToken)
            .ContinueWith(task => (IReadOnlyList<ExportCost>)task.Result, cancellationToken);

    public void AddCost(ExportCost cost) => dbContext.ExportCosts.Add(cost);

    public Task<IReadOnlyList<ExportOrderStatusEvent>> ListTimelineAsync(Guid organizationId, Guid orderId, CancellationToken cancellationToken = default) =>
        dbContext.ExportOrderStatusEvents.AsNoTracking().Where(x => x.OrganizationId == organizationId && x.OrderId == orderId)
            .OrderBy(x => x.OccurredOn).ThenBy(x => x.CreatedAtUtc).ToListAsync(cancellationToken)
            .ContinueWith(task => (IReadOnlyList<ExportOrderStatusEvent>)task.Result, cancellationToken);
    ...
    public Task<decimal> SumCostsBrlAsync(Guid organizationId, IReadOnlyCollection<Guid> orderIds, CancellationToken cancellationToken = default) =>
        dbContext.ExportCosts.AsNoTracking().Where(x => x.OrganizationId == organizationId && orderIds.Contains(x.OrderId))
            .SumAsync(x => (decimal?)x.AmountBrl, cancellationToken).ContinueWith(task => task.Result ?? 0m, cancellationToken);
